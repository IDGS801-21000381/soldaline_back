    ...
    public class EmpresaClienteController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetResumenVista()
        {
            var clientesVista = await _context.ClientePotencials
                ...
                .Select(c => new
                {
                    ClienteId = c.ClienteId, // Ahora incluye el ID de cliente
                    ...
                    Estatus = c.Estatus,
                    EmpresaId = c.EmpresaId // También incluye el ID de la empresa
                })
                .ToListAsync();
        ...
        [HttpPut("change-status/{clienteId}")]
        public async Task<IActionResult> ChangeStatus(int clienteId, [FromBody] EstatusUpdateDTO dto)
        {
            // Validar estatus de Cliente
            if (!new[] { 1, 2, 3, 4 }.Contains(dto.EstatusCliente))
            {
                return BadRequest("Estatus de cliente inválido. Los valores permitidos son 1 (Activo), 2 (Inactivo), 3 (Pendiente), 4 (Cancelado).");
            }

            // Buscar Cliente
            var cliente = await _context.ClientePotencials.FirstOrDefaultAsync(c => c.ClienteId == clienteId);
            if (cliente == null)
            {
            ...
            cliente.Estatus = dto.EstatusCliente;

            await _context.SaveChangesAsync();
            return Ok("Estatus de cliente actualizado correctamente.");
        }
        ...
        public async Task<IActionResult> SearchClientes(string searchTerm)
        {
            var clientes = await _context.ClientePotencials
                ...
                .Select(c => new
                {
                    ClienteId = c.ClienteId, // Incluir ClienteId
                    NombreCliente = c.Nombre,
                    NombreEmpresa = c.Empresa != null ? c.Empresa.Nombre : "Sin Empresa",
                    Correo = c.Correo,
                    Telefono = c.Telefono,
                    Direccion = c.Direccion,
                    Estatus = c.Estatus,
                    EmpresaId = c.EmpresaId // Incluir EmpresaId
                })
                .ToListAsync();
